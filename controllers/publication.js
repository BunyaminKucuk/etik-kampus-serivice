...
function getMyPublication(req, res) {
    var page = 1;
    ...
    Publication.find({ 'user': req.user.sub, '_id': publicationId }).populate('followed').exec((err, follows) => {
        if (err)
...
function getPublications(req, res) {
    ...
    Follow.find({ user: req.user.sub }).populate('followed').exec((err, follows) => {
        ...
        follows_clean.push(req.user.id);
        Publication.find({ user: { "$in": follows_clean } }).sort('-created_at').populate('user').paginate(page, itemsPerPage, (err, publications, total) => {
            if (err)
                return res.status(500).send({ message: "Get publications error..." });
            if (!publications)
                ...
                if (publication) {
                    Publication.findByIdAndUpdate(publicationId, { file: file_name }, { new: true }, (err, publicationUpdated) => {
                        ...
                        if (!publicationUpdated)
                            return res.status(404).send({ message: "Publication Not Found." });
                        if (err)
                            return res.status(500).send({ message: "asdadadsadadassa." });
